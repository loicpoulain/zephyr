...
#include "nrf_legacy_dfu.h"

/*
 * The Device Firmware Update (DFU) Profile is a proprietary profile that can be
 * used to update firmware files using Bluetooth low energy. This profile is not
 * a standard Bluetooth SIG profile. It is defined by Nordic Semiconductor.
 * It has been renamed and supported as 'Legacy DFU' since the introduction of
 * Secure DFU in nRF5 SDK v12.
 *
 * ref: https://infocenter.nordicsemi.com/topic/com.nordic.infocenter.sdk5.v11.0.0/bledfu_transport_bleprofile.html
 */
...
static struct nrf_legacy_dfu_cb *cb;
...
BT_GATT_SERVICE_DEFINE(nrf_legacy_dfu_svc,
			    ...
			    u8_t flags)
{
	...
	case DFU_STATE_WAIT_START_PKT:
	{
		...
		bt_gatt_notify(conn, &nrf_legacy_dfu_svc.attrs[1],
			       &rsp, sizeof(rsp));
	...
	case DFU_STATE_WAIT_INIT_PKT:
	{
		...
		bt_gatt_notify(conn, &nrf_legacy_dfu_svc.attrs[1], &rsp,
			       sizeof(rsp));
...
void nrf_legacy_dfu_init(struct nrf_legacy_dfu_cb *callbacks, u16_t type,
			 u16_t revision)
{
	cb = callbacks;
	device_type = type;
