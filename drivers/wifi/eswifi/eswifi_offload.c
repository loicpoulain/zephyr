...
static int __select_socket(struct eswifi_dev *eswifi, uint8_t idx)
{
	int err;

	sprintf(eswifi->buf, "P0=%d\r", idx);
...
static int __read_data(struct eswifi_dev *eswifi, size_t len)
{
	...
	/* Set max read size */
	sprintf(size, "R1=%u\r", len);
	err = eswifi_request(eswifi, size, strlen(size),
			     eswifi->buf, sizeof(eswifi->buf));
	if (err || !__is_at_ok(eswifi->buf)) {
		SYS_LOG_ERR("Unable to set read size");
		return -EIO;
	}

	/* Set timeout */
	sprintf(timeout, "R2=%u\r", 30); /* 30 ms */
	err = eswifi_request(eswifi, timeout, strlen(timeout),
			     eswifi->buf, sizeof(eswifi->buf));
	if (err || !__is_at_ok(eswifi->buf)) {
		SYS_LOG_ERR("Unable to set timeout");
		return -EIO;
	}
			  ...
			  struct net_context **context)
{
	...
	if (ip_proto != IPPROTO_TCP) {
		/* TODO: add UDP */
		SYS_LOG_ERR("Only TCP supported");
	...
	if (!socket) {
		SYS_LOG_ERR("No socket resource available");
	...
	if (err) {
		SYS_LOG_ERR("Unable to select socket %u", socket->index);
		eswifi_unlock(eswifi);
		return -EIO;
	}

	/* Set Transport Protocol */
	sprintf(eswifi->buf, "P1=%d\r", ESWIFI_TRANSPORT_TCP);
	err = eswifi_request(eswifi, eswifi->buf, strlen(eswifi->buf),
			     eswifi->buf, sizeof(eswifi->buf));
	if (err || !__is_at_ok(eswifi->buf)) {
		SYS_LOG_ERR("Unable to set transport protocol");
		eswifi_unlock(eswifi);
		return -EIO;
	}
			   ...
			   socklen_t addrlen)
{
	...
	SYS_LOG_DBG("%u", net_sin(addr)->sin_port);
	...
	/* Set Local Port */
	sprintf(eswifi->buf, "P2=%d\r", net_sin(addr)->sin_port);
	err = eswifi_request(eswifi, eswifi->buf, strlen(eswifi->buf),
			     eswifi->buf, sizeof(eswifi->buf));
	if (err || !__is_at_ok(eswifi->buf)) {
		SYS_LOG_ERR("Unable to set local port");
		eswifi_unlock(eswifi);
		return -EIO;
	}
			      ...
			      void *user_data)
{
	...
	SYS_LOG_DBG("%u.%u.%u.%u:%u", sin_addr->s4_addr[0],
		    sin_addr->s4_addr[1], sin_addr->s4_addr[2],
		    sin_addr->s4_addr[3], port);

	eswifi_lock(eswifi);

	/* Set Remote IP */
	sprintf(eswifi->buf, "P3=%u.%u.%u.%u\r", sin_addr->s4_addr[0],
		sin_addr->s4_addr[1], sin_addr->s4_addr[2],
		sin_addr->s4_addr[3]);
	...
	err = eswifi_request(eswifi, eswifi->buf, strlen(eswifi->buf),
			     eswifi->buf, sizeof(eswifi->buf));
	if (err || !__is_at_ok(eswifi->buf)) {
		SYS_LOG_ERR("Unable to set remote ip");
		eswifi_unlock(eswifi);
		return -EIO;
	}

	/* Set Remote Port */
	sprintf(eswifi->buf, "P4=%d\r", net_sin(addr)->sin_port);
	err = eswifi_request(eswifi, eswifi->buf, strlen(eswifi->buf),
			     eswifi->buf, sizeof(eswifi->buf));
	if (err || !__is_at_ok(eswifi->buf)) {
		SYS_LOG_ERR("Unable to set remote port");
		eswifi_unlock(eswifi);
		return -EIO;
	}

	/* Start TCP client */
	sprintf(eswifi->buf, "P6=1\r");
	err = eswifi_request(eswifi, eswifi->buf, strlen(eswifi->buf),
			     eswifi->buf, sizeof(eswifi->buf));
	if (err || !__is_at_ok(eswifi->buf)) {
		SYS_LOG_ERR("Unable to connect");
		eswifi_unlock(eswifi);
		return -EIO;
	}
	SYS_LOG_ERR("%s", eswifi->buf);
			   ...
			   void *user_data)
{
	...
	/* header */
	sprintf(eswifi->buf, "S3=%u\r", bytes);
	...
	err = eswifi_request(eswifi, eswifi->buf, offset + 1,
			     eswifi->buf, sizeof(eswifi->buf));
	if (err || !__is_at_ok(eswifi->buf)) {
		SYS_LOG_ERR("Unable to send data");
		eswifi_unlock(eswifi);
		return -EIO;
	}
...
static void eswifi_off_read_data(struct eswifi_dev *eswifi)
{
	...
	for (i = 0; i < ESWIFI_OFFLOAD_MAX_SOCKETS; i++) {
		...
		err = __read_data(eswifi, 1460); /* 1460 is max size */
		if (err || !socket->recv_cb) {
			continue;
		}

		pkt = net_pkt_get_reserve_rx(0, K_NO_WAIT);
		if (!pkt) {
			SYS_LOG_ERR("Cannot allocate rx packet");
			continue;
		}

		frag = net_pkt_get_frag(pkt, K_NO_WAIT);
		if (!frag) {
			SYS_LOG_ERR("Cannot allocate rx frag");
			net_pkt_unref(pkt);
			continue;
		}
		...
		if (!net_pkt_append_all(pkt, 1460, eswifi->buf, K_NO_WAIT)) {
			SYS_LOG_WRN("Incomplete buffer copy");
		}
...
int eswifi_offload_init(struct eswifi_dev *eswifi)
{
	...
	if (err) {
		SYS_LOG_ERR("Unable to configure dhcp");
		return err;
	}

	err = eswifi_off_disable_bypass(eswifi);
	if (err) {
		SYS_LOG_ERR("Unable to disable bypass mode");
		return err;
	}
