...
struct eswifi_bus_ops {
	...
	int (*write)(struct eswifi_dev *eswifi, char *data, size_t dlen);
	int (*read)(struct eswifi_dev *eswifi, char *data, size_t dlen);
};

static inline int eswifi_request(struct eswifi_dev *eswifi, char *cmd, size_t clen,
				 char *rsp, size_t rlen)
{
	int ret;

	ret = eswifi->bus->request(eswifi, cmd, clen, rsp, rlen);
